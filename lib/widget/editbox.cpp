...
void W_EDITBOX::fitStringStart()
{
	...
	while (!tmp.isEmpty())
	{
		int pixelWidth = iV::GetTextWidth(tmp.toUtf8().constData());
...
void W_EDITBOX::display(int xOffset, int yOffset)
{
	...
	iV::DrawText(tmp.toUtf8().constData(), fx, fy);
...
#endif
	{
		...
		int cx = x0 + WEDB_XGAP + iV::GetTextWidth(tmp.toUtf8().constData());
		cx += iV::GetTextWidth("-");
		int cy = fy;
		iV_Line(cx, cy + iV::GetTextAboveBase(), cx, cy - iV::GetTextBelowBase(), WZCOL_FORM_CURSOR);
	}
#if CURSOR_BLINK
