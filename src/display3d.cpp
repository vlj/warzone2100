...
static void NetworkDisplayImage(WIDGET *psWidget, UDWORD xOffset, UDWORD yOffset)
{
	...
	if (NETcheckPlayerConnectionStatus(status, NET_ALL_PLAYERS))
	{
		...
		width = iV::GetTextWidth(players) + 10;
		height = iV::GetTextHeight(players) + 10;

		iV::SetTextColour(WZCOL_TEXT_BRIGHT);
		iV::DrawText(players, x - width, y + height);
	}
...
void draw3DScene(void)
{
	...
	if (showSAMPLES)		//Displays the number of sound samples we currently have
	{
		...
		width = iV::GetTextWidth(Qbuf) + 11;
		height = iV::GetTextHeight(Qbuf);

		iV::DrawText(Qbuf, pie_GetVideoBufferWidth() - width, height + 2);
		iV::DrawText(Lbuf, pie_GetVideoBufferWidth() - width, height + 48);
		iV::DrawText(Abuf, pie_GetVideoBufferWidth() - width, height + 59);
	}
	if (showFPS)
	{
		iV::SetFont(iV::fonts::font_regular);
		...
		width = iV::GetTextWidth(fps) + 10;
		height = iV::GetTextHeight(fps);

		iV::DrawText(fps, pie_GetVideoBufferWidth() - width, pie_GetVideoBufferHeight() - height);
	}
	if (showORDERS)
	{
		iV::SetFont(iV::fonts::font_regular);
		unsigned int height;
		height = iV::GetTextHeight(DROIDDOING);
		iV::DrawText(DROIDDOING, 0, pie_GetVideoBufferHeight() - height);
	}
	...
	if (getWidgetsStatus() && !gamePaused())
	{
		...
		iV::DrawText(getLevelName(), RET_X + 134, 410 + E_H);
		getAsciiTime(buildInfo, graphicsTime);
		iV::DrawText(buildInfo, RET_X + 134, 422 + E_H);
		if (getDebugMappingStatus())
		{
			iV::DrawText("DEBUG ", RET_X + 134, 436 + E_H);
		}
	}
