...
static int playerBoxHeight(int player)
{
	...
	gap = std::min(gap, 5 * gapDiv);
	static_assert(MULTIOP_TEAMSHEIGHT == MULTIOP_PLAYERHEIGHT, "Wrong MULTIOP_TEAMSHEIGHT");  // Why are these different defines?
...
static void processMultiopWidgets(uint32_t id)
{
	...
	static_assert(MULTIOP_TEAMS_START + MAX_PLAYERS - 1 <= MULTIOP_TEAMS_END, "Wrong MULTIOP_TEAMS_START");
	...
	//clicked on a team
	static_assert(MULTIOP_TEAMCHOOSER + MAX_PLAYERS - 1 <= MULTIOP_TEAMCHOOSER_END, "Wrong MULTIOP_TEAMCHOOSER");
	...
	// clicked on a player
	static_assert(MULTIOP_PLAYER_START + MAX_PLAYERS - 1 <= MULTIOP_PLAYER_END, "Wrong MULTIOP_PLAYER_START");
	...
	static_assert(MULTIOP_COLCHOOSER + MAX_PLAYERS - 1 <= MULTIOP_COLCHOOSER_END, "Wrong MULTIOP_COLCHOOSER");
...
void displayColour(WIDGET *psWidget, uint32_t xOffset, uint32_t yOffset)
{
	...
	if (NetPlay.players[j].wzFiles.empty() && game.skDiff[j])
	{
		...
		static_assert(MAX_PLAYERS <= 16, "Wrong MAX_PLAYERS");
