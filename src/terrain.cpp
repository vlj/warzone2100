...
static void updateSectorGeometry(int x, int y)
{
	...
	geometryVBO->upload(sizeof(RenderVertex)*sectors[x * ySectors + y].geometryOffset,
		sizeof(RenderVertex)*sectors[x * ySectors + y].geometrySize, geometry);
	waterVBO->upload(sizeof(RenderVertex)*sectors[x * ySectors + y].waterOffset,
	...
	decalVBO->upload(sizeof(DecalVertex)*sectors[x * ySectors + y].decalOffset,
...
//	glGetIntegerv(GL_MAX_ELEMENTS_INDICES,  &GLmaxElementsIndices);
	GLmaxElementsVertices = 32000;
	GLmaxElementsIndices = 32000;
...
#define q(i,j,center) ((x*ySectors+y)*(sectorSize+1)*(sectorSize+1)*2 + ((i)*(sectorSize+1)+(j))*2+(center))
	...
	if (geometryVBO)
		delete geometryVBO;
	geometryVBO = gfx_api::context::get().create_buffer(gfx_api::buffer::usage::vertex_buffer, sizeof(RenderVertex)*geometrySize);
	geometryVBO->upload(0, sizeof(RenderVertex)*geometrySize, geometry);
	free(geometry);

	if (geometryIndexVBO)
		delete geometryIndexVBO;
	geometryIndexVBO = gfx_api::context::get().create_buffer(gfx_api::buffer::usage::index_buffer, sizeof(GLuint)*geometryIndexSize);
	geometryIndexVBO->upload(0, sizeof(GLuint)*geometryIndexSize, geometryIndex);
	free(geometryIndex);

	if (waterVBO)
		delete waterVBO;
	waterVBO = gfx_api::context::get().create_buffer(gfx_api::buffer::usage::vertex_buffer, sizeof(RenderVertex)*waterSize);
	waterVBO->upload(0, sizeof(RenderVertex)*waterSize, water);
	free(water);

	if (waterIndexVBO)
		delete waterIndexVBO;
	waterIndexVBO = gfx_api::context::get().create_buffer(gfx_api::buffer::usage::index_buffer, sizeof(GLuint)*waterIndexSize);
	waterIndexVBO->upload(0, sizeof(GLuint)*waterIndexSize, waterIndex);
	free(waterIndex);
	...
	if (textureVBO)
		delete textureVBO;
	textureVBO = gfx_api::context::get().create_buffer(gfx_api::buffer::usage::vertex_buffer, sizeof(PIELIGHT)*xSectors * ySectors * (sectorSize + 1) * (sectorSize + 1) * 2 * numGroundTypes);
	textureVBO->upload(0, sizeof(PIELIGHT)*xSectors * ySectors * (sectorSize + 1) * (sectorSize + 1) * 2 * numGroundTypes, texture);
	free(texture);

	if (textureIndexVBO)
		delete textureIndexVBO;
	textureIndexVBO = gfx_api::context::get().create_buffer(gfx_api::buffer::usage::index_buffer, sizeof(GLuint)*textureIndexSize);
	textureIndexVBO->upload(0, sizeof(GLuint)*textureIndexSize, textureIndex);
	free(textureIndex);
	...
	if (decalVBO)
		delete decalVBO;
	decalVBO = gfx_api::context::get().create_buffer(gfx_api::buffer::usage::vertex_buffer, sizeof(DecalVertex)*decalSize);
	decalVBO->upload(0, sizeof(DecalVertex)*decalSize, decaldata);
	free(decaldata);
	...
	lightmap_tex_num = gfx_api::context::get().create_texture(1, lightmapWidth, lightmapHeight, gfx_api::texel_format::FORMAT_RGB8_UNORM_PACK8);

	lightmap_tex_num->upload(0, 0, 0, lightmapWidth, lightmapHeight, gfx_api::texel_format::FORMAT_RGB8_UNORM_PACK8, lightmapPixmap);
...
static void drawTerrainLayers(const glm::mat4 &ModelViewProjection, const glm::vec4 &paramsXLight, const glm::vec4 &paramsYLight, const glm::mat4 &textureMatrix)
{
	...
	for (int layer = 0; layer < numGroundTypes; layer++)
	{
		...
		gfx_api::TerrainLayer::get().bind_constants({ ModelViewProjection, paramsX, paramsY, paramsXLight, paramsYLight, glm::mat4(), textureMatrix,
			renderState.fogEnabled, renderState.fogBegin, renderState.fogEnd, fogColor, 0, 1 });
...
void drawWater(const glm::mat4 &viewMatrix)
{
	...
	gfx_api::WaterPSO::get().bind_constants({ viewMatrix, paramsX, paramsY, paramsX2, paramsY2,
		glm::translate(waterOffset, 0.f, 0.f), glm::mat4(), renderState.fogEnabled, renderState.fogBegin, renderState.fogEnd, glm::vec4{}, 0, 1
});
